...
RUN apt-get update && apt-get install -y   libudunits2-dev libgdal-dev libgeos-dev libproj-dev libmysqlclient-dev && rm -rf /var/lib/apt/lists/*
...
EXPOSE 80
CMD R -e "options('shiny.port'=80,shiny.host='0.0.0.0');library(refineryanalysis);refineryanalysis::run_app()"
